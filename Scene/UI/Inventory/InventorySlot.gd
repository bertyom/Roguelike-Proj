...
#Quick transfer with ctrl+click
	if  ContainerLoot.current_container_ID != null and CommonFunctions.UI_node.get_node_or_null("Container"):
...
func _on_Icon_mouse_entered():
	if PlayerData.inv_data[get_parent().get_name()]["Item"] != null:
		#if SceneHandler.close_tooltip.is_connected(_on_close_tooltip) == false:
			#SceneHandler.close_tooltip.connect(_on_close_tooltip)
